  ...
  <div class="execution">
    <basic-container>
      ...
      <avue-crud ref="crud"
        ...
        <span v-if="scope.row.status==0" style="color:red">未建档</span>
        <span v-if="scope.row.status==1" style="color:#00ec00">已建档</span>
        </template>         
        <template slot-scope="scope" slot="menuLeft">
            <el-radio v-model="radio2" label="" border size="medium" @change="handleZero">全部</el-radio>
            <el-radio v-model="radio2" label="0" border size="medium" @change="handleSelect">未建档</el-radio>
            <el-radio v-model="radio2" label="1" border size="medium" @change="handleOnlod">已建档</el-radio>
        </template>
...
<script>
import {fetchList,putObj} from '@/api/admin/newFile'         //引入接口
import {tableOption} from '@/const/crud/admin/newFile'         //引入表格头
